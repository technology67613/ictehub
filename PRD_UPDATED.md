...
- `/apply` → AdmissionForm (multi-step nursing & higher ed admission form)
- `/login` → AuthPage (tabbed student & staff login — student default password is DOB in DDMMYYYY format)
...
### Protected routes
- `/student/dashboard` → StudentDashboard (student role only — application status, documents, course details, timeline)
- `/student/profile` → StudentProfile (student role only)
- `/profile` → ProfilePage (staff / admin / telecaller)
...
| name | text | |
| email | text | unique, required (students: `[phone]@student.ictehub`) |
| password_hash | text | bcrypt, required (student default: DOB in `DDMMYYYY`) |
| role | text | enum: `admin`, `telecaller`, `student` |
...
| student_user_id | uuid | nullable, references users(id) ON DELETE SET NULL |
| access_token | uuid | default gen_random_uuid(), unique index |
| admission_form_data | text / jsonb | structured multi-step form responses |
...
| document_type | text | passport_photo, marksheet_10th, marksheet_12th, id_proof, etc. |
| document_name | text | display filename |
| file_url | text | Supabase Storage URL |
| file_size | numeric | size in bytes |
