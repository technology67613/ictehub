...
const STATUS_CONFIG = {
  ...
  'enrolled':       { label: 'Enrolled',       color: '#10B981', bg: '#ECFDF5', icon: Award },
};
...
// ─── Drawer ──────────────────────────────────────────────────────────────────
function LeadDrawer({ lead, telecallers, onClose, onAssign, savingId, savedFlash, copyToClipboard }) {
  const status = STATUS_CONFIG[lead.status] || STATUS_CONFIG['new'];
  const courseName = lead.institute_courses?.name || '—';
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-start gap-3 p-3 bg-[#EEF2FF] rounded-xl border border-[#1E40FF]/10">
              ...
              <div>
                <div className="text-[9px] font-bold text-[#1E40FF] uppercase tracking-widest">Interested Program</div>
                <div className="text-sm font-semibold text-[#1E40FF] mt-0.5">{courseName}</div>
              </div>
            </div>
            {lead.message && (
              <div className="flex items-start gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div>
                  <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Student Remarks</div>
                  <div className="text-xs font-semibold text-slate-600 mt-1 leading-relaxed">{lead.message}</div>
                </div>
              </div>
            )}
          </div>
...
// ─── Main AdminInstituteLeads Component ──────────────────────────────────────
export default function AdminInstituteLeads({ token }) {
  ...
  const fetchLeads = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API}/institute-leads`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!response.ok) throw new Error('Failed to fetch institute leads');
  ...
  const handleAssignTelecaller = async (leadId, telecallerId) => {
    setSavingId(leadId);
    try {
      const response = await fetch(`${API}/institute-leads/${leadId}`, {
        method: 'PUT',
        headers: {
  ...
  const exportToCSV = () => {
    const headers = ['Name', 'Phone', 'Email', 'Course', 'Status', 'Assigned Telecaller', 'Message', 'Created Date'];
    ...
    link.setAttribute('download', `institute_leads_export_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter(lead => {
      ...
      if (statusFilter === 'enrolled') return lead.status === 'enrolled';
      return true;
    });
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="hidden md:grid md:grid-cols-[1.5fr_1fr_1.2fr_0.8fr_1.2fr_auto] gap-3 px-4 py-3 bg-slate-50 border-b border-slate-100">
            ...
            <div className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Course</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full md:contents">
                      ...
                      {/* Course */}
                      <div className="flex flex-col gap-0.5 min-w-0">
                        <span className="md:hidden text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Course</span>
